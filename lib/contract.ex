defmodule Harmony.Contract do
    ...
    def handle_call({:get_filter_changes, filter_id}, _from, state) do
      ...
      {:ok, logs} = Harmony.get_filter_changes(filter_id)
    ...
    #   {:ok, receipt} = Harmony.get_transaction_receipt_by_hash(tx_hash)
    ...
    #           Enum.zip(
    #             event_attributes[:non_indexed_names],
    #             Harmony.decode_abi_event(log["data"], event_attributes[:signature])
    #           )
    #           |> Enum.into(%{})
    #         if length(log["topics"]) > 1 do
    #           [_head | tail] = log["topics"]
